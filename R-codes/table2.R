...
# function to compute actual grided  contour
...
for (j in 1:11){
  dt=NULL
  dt=readRDS(file=file1[j])             # a contours of a year 
  n1[j]=ncol(dt)
...
dt1=which(st==1)                                # all censored regions
dt2=which(st==2)                                # all complete regions
dt11=intersect(dt1,dt4)                         # range (> 200 sq km && <13500) censored contour
dt22=intersect(dt2,dt4)[-80]                    
dt22=intersect(dt2,dt4)                         # range (> 200 sq km && <13500) complete spell (except one or two) # CHANGINH
N1=length(dt22)                                 # number of complete observations
N2=length(dt11)                                 # number of censored observations
time1=NULL
censor1=NULL
dt44=c(dt11,dt22)
time1=ar[dt44]                                 # contours which lies in the given range
censor1=st[dt44]
...
h10=setdiff(c(1:length(x0)),h1)                               # the remaining out observations
...
cov1=(tcrossprod((data_star[,order1]-mean_x)*(matrix(sqrt(wts), nrow = s, ncol = l3, byrow = TRUE))))  #  This is not covarince
...
# SRFV Computation
# Prerequisite functions (obtained from FDASRVF package of R)
...
h11=eigen(K)$values  
hv1=eigen(K)$vectors  
MM1=290                   # Taking out first 1705 and 290 PCS
...
best=which(fda_error==min(fda_error))
med1=intersect(which(c1[,2]>(median(c1[,2])-0.00005)),which(c1[,2]<(median(c1[,2])+0.00005))) # Median reconstruction
best=which(fda_error==min(fda_error))
